...
bool MineSweeper::openField(int x, int y) {
    ...
    if (m_gamefield[idx].opened) return false;

    m_gamefield[idx].opened = true;

    if (m_gamefield[idx].bomb) return true;

    for (int dy = -1; dy <= 1; dy++) {
        if ((y + dy < 0) || (y + dy >= h)) continue;

        for (int dx = -1; dx <= 1; dx++) {
            if ((x + dx < 0) || (x + dx >= w) || ((dx == 0) && (dy == 0))) continue;
...
void MineSweeper::draw(Image& frame) {
    ...
    for (size_t y = 0; y < frame.height; y++) {
        ...
        if ((y >= barOffset) && (y <= NEIGHBOR_CNT + barOffset)) *p = y - barOffset;
        p += PAD_X;
