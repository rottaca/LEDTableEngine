...
void MatrixScrollText::init(size_t height, size_t width){
    m_width = width;
    m_height = height;
    resetScrollState();
}
...
bool MatrixScrollText::update(TimeUnit currTime){
  if((m_currScrollXPixels != 0 && m_currScrollXPixels != m_scrollXPixels && currTime-m_timeTextWarpStart >= m_timeTextWarpPerChar) ||
     ((m_currScrollXPixels == 0 || m_currScrollXPixels == m_scrollXPixels) && currTime-m_timeTextWarpStart >= m_timeTextWarpPerChar + m_timeTextWarpInitialWait))
   {
