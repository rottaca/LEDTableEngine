...
void Snake::processInput(const BaseInput::InputEvents &events,
	...
	if(m_deathTime > 0 && m_ctrl->getTimeMs() - m_deathTime > 3000){
      m_hasFinished = true;
      return;
	}
	...
	if(m_ctrl->getTimeMs() - m_lastUpdateTime < 1000.0/(0.2*m_snake.size())){
	...
	if(newPos.x < 0 || newPos.x > m_ctrl->getWidth() ||
	   ...
	   (std::find(m_snake.begin(), m_snake.end())!= m_snake.end()){
		   m_deathTime = m_ctrl->getTimeMs();
   }else{
	   m_snake.push_front(newPos);
	   
	   if(newPos == m_foodPos){
		   m_foodPos.x = m_posDist(m_generator);
		   m_foodPos.y = m_posDist(m_generator);
	   }else{
		   m_snake.pop_back();
	   }
   }	
}
void Snake::draw(std::vector<uint8_t> &frame){
	if(m_deathTime > 0){
		m_ctrl->clearFrame(3);	
	}else{
		m_ctrl->clearFrame(0);		
	...
	frame[m_foodPos.x + m_foodPos.y*m_ctrl->getWidth()] = 1;
	
	for(Point p: m_snake){
		frame[p.x + p.y*m_ctrl->getWidth()] = 2;
	}
}
